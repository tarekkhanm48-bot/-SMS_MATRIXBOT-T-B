...
BOT_TOKEN   = '8610107064:AAEwIpunMmd-fLGBLmOxqCKp9b9RxrQ5fkw'
SUPER_ADMIN = 8048890397
OTP_GROUP   = -1003926163328   # numeric supergroup ID — always works
BOT_FILE    = os.path.abspath(__file__)

# ════════════════════════════════════════════════════════════
#  FASTXOTPS API KEYS — অটো-রোটেশন (Key1 → Key2 → Key3)
# ════════════════════════════════════════════════════════════
FASTX_API_KEYS = [
    "MURAD_2488747B8CD70162AE7D5D6E",                              # ✅ Primary (Active #3723)
    "SVJUQz1SS36GgYV6dH9URg==",                                    # 🔁 Backup-1
    "RlZVR0NBUzRIZoCGg06WYnN2T1JIVXFbYmSEYH9pdImDZnVldHFShw==",  # 🔁 Backup-2
]
FASTX_BASE    = "https://fastxotps.com"
RENDER_DOMAIN = os.environ.get(
    'RENDER_EXTERNAL_HOSTNAME',
    os.environ.get('WEBHOOK_DOMAIN', 'your-app.onrender.com')
)
_FASTX_HEADERS = {
...
def _fastx_get_number(service: str, country: str):
    """৩টি কি পর্যায়ক্রমে চেষ্টা করে নম্বর নেয়।"""
    for idx, key in enumerate(FASTX_API_KEYS, 1):
        try:
            r = http_requests.post(
                f"{FASTX_BASE}/get_number",
                json={"api_key": key, "service": service, "country": country},
                headers=_FASTX_HEADERS, timeout=15)
            if r.status_code == 200:
                d = r.json()
                if d.get("status") not in ("error", "failed", "no_numbers", None):
                    return d, idx
        except Exception:
            pass
    return {"status": "failed", "error": "সব API কি ব্যর্থ হয়েছে"}, 0

def _fastx_check_otp(request_id: str):
    """৩টি কি দিয়ে OTP চেক করে।"""
    for idx, key in enumerate(FASTX_API_KEYS, 1):
        try:
            r = http_requests.post(
                f"{FASTX_BASE}/get_otp",
                json={"api_key": key, "request_id": request_id},
                headers=_FASTX_HEADERS, timeout=15)
            if r.status_code == 200:
                d = r.json()
                if d.get("status") not in ("error", "failed", None):
                    return d, idx
        except Exception:
            pass
...
Thread(target=lambda: flask_app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 8000))), daemon=True).start()
...
# ════════════════════════════════════════════════════════════
#  /getnum — fastxotps.com থেকে সরাসরি নম্বর নিন (অটো-রোটেশন)
# ════════════════════════════════════════════════════════════
@bot.on(events.NewMessage(pattern=r'^/getnum(?: (.+))?$'))
async def on_getnum(event):
    if event.is_channel: return
    uid  = event.sender_id
    args = (event.pattern_match.group(1) or '').strip().split()
    if len(args) < 2:
        await event.respond(
            "📖 **ব্যবহার:** `/getnum <service> <country>`\n\n"
            "**উদাহরণ:**\n"
            "`/getnum whatsapp bd`\n"
            "`/getnum telegram in`\n"
            "`/getnum facebook pk`\n\n"
            "**Country কোড:**\n"
            "`bd` বাংলাদেশ | `in` ভারত | `pk` পাকিস্তান\n"
            "`my` মালয়েশিয়া | `id` ইন্দোনেশিয়া | `us` আমেরিকা",
            parse_mode='md'); return
    service = args[0].lower()
    country = args[1].lower()
    wait = await event.respond(f"⏳ `{service}` / `{country}` — নম্বর খোঁজা হচ্ছে…", parse_mode='md')
    result, key_used = await asyncio.get_event_loop().run_in_executor(
        None, _fastx_get_number, service, country)
    if key_used == 0 or result.get("status") in ("error", "failed", "no_numbers"):
        err = result.get("error", result.get("message", "অজানা কারণ"))
        await wait.edit(
            f"❌ **নম্বর পাওয়া যায়নি**\n\n🔧 কারণ: `{err}`\n"
            "💡 অন্য service বা country চেষ্টা করুন।", parse_mode='md'); return
    number     = result.get("number", result.get("phone", "N/A"))
    request_id = result.get("request_id", result.get("id", "N/A"))
    await wait.edit(
        "✅ **নম্বর পাওয়া গেছে!**\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"📱 **Number:**     `{number}`\n"
        ...
        f"🔑 **API Key:**    Key-{key_used}\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"OTP চেক করতে: `/checkotp {request_id}`", parse_mode='md')

# ════════════════════════════════════════════════════════════
#  /checkotp — Request ID দিয়ে OTP চেক করুন
# ════════════════════════════════════════════════════════════
@bot.on(events.NewMessage(pattern=r'^/checkotp(?: (.+))?$'))
async def on_checkotp(event):
    if event.is_channel: return
    rid = (event.pattern_match.group(1) or '').strip()
    if not rid:
        await event.respond("📖 ব্যবহার: `/checkotp <request_id>`", parse_mode='md'); return
    wait = await event.respond(f"⏳ OTP চেক করা হচ্ছে… ID: `{rid}`", parse_mode='md')
    result, key_used = await asyncio.get_event_loop().run_in_executor(
        None, _fastx_check_otp, rid)
    otp = result.get("otp", result.get("code", None))
    if not otp:
        await wait.edit(
            "⏳ **OTP এখনো আসেনি**\n\nকিছুক্ষণ পর আবার চেষ্টা করুন।", parse_mode='md'); return
    await wait.edit(
        "🎉 **OTP পাওয়া গেছে!**\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"🔑 **OTP CODE:** `{otp}`\n"
...
# ════════════════════════════════════════════════════════════
#  /webhook — Render Webhook URL দেখুন (Admin only)
# ════════════════════════════════════════════════════════════
@bot.on(events.NewMessage(pattern=r'^/webhook$'))
async def on_webhook_url(event):
    if event.sender_id != SUPER_ADMIN: return
    wh = f"https://{RENDER_DOMAIN}/api/webhook/otp"
    await event.respond(
        f"🌐 **Render Webhook URL:**\n\n`{wh}`\n\n"
        "⬆️ fastxotps.com → Profile → **Telegram Push** সেকশনে বসান।",
        parse_mode='md')
...
async def on_cb(event):
    ...
    if data.startswith("opn_"):
        ...
        if pt == "ivasms":
            ...
            domain = os.environ.get('RENDER_EXTERNAL_HOSTNAME', os.environ.get('WEBHOOK_DOMAIN', 'your-render-domain.onrender.com'))
